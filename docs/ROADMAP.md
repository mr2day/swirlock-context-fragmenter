...
| Per-turn hallucination index + active/sleep state machine | done | Unit H: `messages.hallucination_index INTEGER` written per assistant turn (fast-path 0 for trivial replies, else full audit pipeline → 0–10 index from verdicts). Gate retired; fragmenter now indexes every turn fire-and-forget on `session.observed`. `ActivityMonitorService` replaces the 30-min setInterval-driven sleep with a quiet-threshold state machine: sleep runs once after 30 min of no activity, then waits for the next `session.observed` to flip back to active. |
